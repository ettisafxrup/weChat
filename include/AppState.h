...
private:
    ...
    User *findCurrentUser()
    {
        if (currentUsername.empty())
            return nullptr;
        for (auto &u : users)
            if (u.getUsername() == currentUsername)
                return &u;
        return nullptr;
    }
...
public:
    ...
    {
        if (!instance)
            instance = new AppState(); // explicit memory allocation
        return *instance;
    }
    ...
    Room *createRoom(const std::string &name, bool isPrivate = false)
    {
        ...
        fm.saveRooms(rooms);
        fm.saveUsers(users);
        // Return pointer to the last element as it need to be stable until next rooms rebuild
        return &rooms.back();
    }

    // Join by 6-char code for function overloading
    ...
    bool sendMessage(const std::string &roomId, const std::string &content)
    {
        ...
        if (!r || !r->hasMember(u->getUsername()))
            return false;
    ...
    bool reloadRoom(const std::string &roomId)
    {
        ...
        {
            if (fr.getRoomId() != roomId)
                continue;
            for (auto &r : rooms)
            {
                if (r.getRoomId() != roomId)
                    continue;
                bool changed = (fr.getMessages().size() != r.getMessages().size() || fr.getMembers().size() != r.getMembers().size());
                if (changed)
                    r = fr; // copy-assign in place — pointer stays valid
                return changed;
            }
    ...
    {
        if (br)
            br->displayInfo(); // virtual dispatch
    }
};
