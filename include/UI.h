...
public:
    ...
    void showChatRoom(const std::string &roomId)
    {
        ...
        while (inRoom)
        {
            ...
            if (needRedraw)
                continue; // go to top — redraw, then poll again
            if (!inputReady)
                continue; // 2 s passed, nothing new — keep waiting
    ...
    void showStats()
    {
        ...
        std::cout
            ...
            << CYAN << "  My rooms joined   : " << RESET << (state.getCurrentUser() ? state.getCurrentUser()->getRoomCount() : 0) << "\n"
